...
class Ti extends MY_Controller
{
    /**
     * author: Ramon Silva
    ...
    public function dashboard()
    {
        ...
        $this->data['menuTi'] = 'dashboard';
        
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'menuTi')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para visualizar esta página');
            redirect(base_url());
        }

        $this->data['proximoChamado'] = $this->ti_model->getProximoChamado('chamados');
        $this->data['beep'] = $this->ti_model->getBeep('chamados');
        $this->data['resultTotalDia'] = $this->ti_model->getChamadosCountDia('chamados');
        $this->data['resultadoMes'] = $this->ti_model->getChamadosMesUsuario('chamados');
        $this->data['tecnico'] = $this->ti_model->getChamadosMaisMes('chamados');
        $this->data['pcEmUso'] = $this->ti_model->getPcUso('parque');
        $this->data['pcGuardado'] = $this->ti_model->getPcGuardado('parque');
        $this->data['pcMarca'] = $this->ti_model->getPcMarca('parque');
        $this->data['pcSSD'] = $this->ti_model->getPcSSD('parque');
        $this->data['pcSemSSD'] = $this->ti_model->getPcSemSSD('parque');
        $this->data['fone'] = $this->ti_model->getFonesUso('parque');
        $this->data['monitor'] = $this->ti_model->getMonitores('parque');
        $this->data['result'] = $this->ti_model->getChamadosCount('chamados');
        //$this->data['resultadoMesMais'] = $this->ti_model->getChamadosMaisMes('chamados');
        
        
        $this->data['view'] = 'ti/dashboard';
        return $this->layout();
    }
     ...
     public function autoCompleteEtiqueta()
    {
        if (isset($_GET['term'])) {
            $q = strtolower($_GET['term']);
            $this->ti_model->autoCompleteEtiqueta($q);
        }
    }
    ...
    public function anexar()
    {
        ...
        $directory = FCPATH . 'assets' . DIRECTORY_SEPARATOR . 'anexos' . DIRECTORY_SEPARATOR . date('m-Y') . DIRECTORY_SEPARATOR . 'OS-' . $this->input->post('idParqueAnexo');
            ...
            if (!$this->upload->do_upload($field_name)) {
                ...
                if ($upload_data['is_image'] == 1) {
                    ...
                    if (!$this->image_lib->resize()) {
                        ...
                        $success[] = $upload_data;
                        $this->load->model('Ti_model');
                        $this->Ti_model->anexar($this->input->post('idParqueAnexo'), $upload_data['file_name'], base_url('assets' . DIRECTORY_SEPARATOR . 'anexos' . DIRECTORY_SEPARATOR . date('m-Y') . DIRECTORY_SEPARATOR . 'OS-' . $this->input->post('idParqueAnexo')), 'thumb_' . $upload_data['file_name'], $directory);
                    }
                } else {
                    $success[] = $upload_data;
                    ...
                    $this->Ti_model->anexar($this->input->post('idParqueAnexo'), $upload_data['file_name'], base_url('assets' . DIRECTORY_SEPARATOR . 'anexos' . DIRECTORY_SEPARATOR . date('m-Y') . DIRECTORY_SEPARATOR . 'OS-' . $this->input->post('idParqueAnexo')), '', $directory);
                }
            }
        }
        ...
        if (count($error) > 0) {
            ...
            log_info('Adicionou anexo(s) a uma OS. ID (OS): ' . $this->input->post('idParqueAnexo'));
            echo json_encode(['result' => true, 'mensagem' => 'Arquivo(s) anexado(s) com sucesso .']);
        }
    }

    public function anexar2()
    {
        $this->load->library('upload');
        $this->load->library('image_lib');

        $directory = FCPATH . 'assets' . DIRECTORY_SEPARATOR . 'anexos' . DIRECTORY_SEPARATOR . date('m-Y') . DIRECTORY_SEPARATOR . 'PC-' . $this->input->post('idParque');
            ...
            if (!$this->upload->do_upload($field_name)) {
                ...
                if ($upload_data['is_image'] == 1) {
                    ...
                    if (!$this->image_lib->resize()) {
                        ...
                        $success[] = $upload_data;
                        $this->load->model('Ti_model');
                        $this->Ti_model->anexar($this->input->post('idParque'), $upload_data['file_name'], base_url('assets' . DIRECTORY_SEPARATOR . 'anexos' . DIRECTORY_SEPARATOR . date('m-Y') . DIRECTORY_SEPARATOR . 'PC-' . $this->input->post('idParque')), 'thumb_' . $upload_data['file_name'], $directory);
                    }
                } else {
                    $success[] = $upload_data;
                    ...
                    $this->Ti_model->anexar($this->input->post('idParque'), $upload_data['file_name'], base_url('assets' . DIRECTORY_SEPARATOR . 'anexos' . DIRECTORY_SEPARATOR . date('m-Y') . DIRECTORY_SEPARATOR . 'PC-' . $this->input->post('idParque')), '', $directory);
                }
            }
        }
        ...
        if (count($error) > 0) {
            ...
            log_info('Adicionou anexo(s) a uma PC. ID (PC): ' . $this->input->post('idParque'));
            echo json_encode(['result' => true, 'mensagem' => 'Arquivo(s) anexado(s) com sucesso .']);
        }
    }
    ...
    ///parte referente ao parque tecnologico
    
    //parque tecnologico levantamento
    public function parque()
    {
        ...
        if ($tipoProduto) {
            $where_array['tipoProduto'] = $tipoProduto;
        }
        if ($de) {
            $de = explode('/', $de);
            $de = $de[2] . '-' . $de[1] . '-' . $de[0];
        ...
        $this->data['configuration']['base_url'] = site_url('ti/parque/');
        $this->data['configuration']['total_rows'] = $this->ti_model->count('parque');

        $this->pagination->initialize($this->data['configuration']);

        $this->data['results'] = $this->ti_model->getParque(
            'parque',
            'parque.*',
            $where_array,
            $this->data['configuration']['per_page'],
        ...
        $this->data['view'] = 'ti/parque';
        return $this->layout();
    }
    
     public function adicionarParque()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'aParque')) {
        ...
        if ($this->form_validation->run('parque') == false) {
            $this->data['custom_error'] = (validation_errors() ? true : false);
        } else {
            $dataInicial = $this->input->post('dataInicial');
            $dataFinal = $this->input->post('dataFinal');
            ...
            $data = [
                ...
                'numeroSerie' => (!empty($this->input->post('numeroSerie'))) ? $this->input->post('numeroSerie') : NULL,
                'marca' => (!empty($this->input->post('marca'))) ? $this->input->post('marca') : NULL,
                'modelo' => (!empty($this->input->post('modelo'))) ? $this->input->post('modelo') : NULL,
                'processador' => (!empty($this->input->post('processador'))) ? $this->input->post('processador') : NULL,
                'memory' => (!empty($this->input->post('memory'))) ? $this->input->post('memory') : NULL,
                'hardDrive' => (!empty($this->input->post('hardDrive'))) ? $this->input->post('hardDrive') : NULL,
                'mac' => (!empty($this->input->post('mac'))) ? $this->input->post('mac') : NULL,
                'office' => (!empty($this->input->post('office'))) ? $this->input->post('office') : NULL,
                'windows' => (!empty($this->input->post('windows'))) ? $this->input->post('windows') : NULL,
                'tipoProduto' => (!empty($this->input->post('tipoProduto'))) ? $this->input->post('tipoProduto') : NULL,
            ];

            if (is_numeric($id = $this->ti_model->add('parque', $data, true))) {
                
                $this->session->set_flashdata('success', 'PC adicionado com sucesso!');
                log_info('Adicionou uma PC');
                redirect(site_url('ti/editarParque/') . $id);
            } else {
                $this->data['custom_error'] = '<div class="alert">Ocorreu um erro.</div>';
            }
        }

        $this->data['view'] = 'ti/adicionarParque';
        return $this->layout();
    }
    
    ///editar parque
    public function editarParque()
    {
        if (!$this->uri->segment(3) || !is_numeric($this->uri->segment(3))) {
            $this->session->set_flashdata('error', 'Item não pode ser encontrado, parâmetro não foi passado corretamente.');
            redirect('mapos');
        ...
        if ($this->form_validation->run('parque') == false) {
            ...
            if ($this->ti_model->edit('parque', $data, 'idParque', $this->input->post('idParque')) == true) {
               
                $this->session->set_flashdata('success', 'Inventário editada com sucesso!');
                log_info('Alterou uma PC. ID: ' . $this->input->post('idParque'));
                redirect(site_url('ti/editarParque/') . $this->input->post('idParque'));
            } else {
                $this->data['custom_error'] = '<div class="form_error"><p>Ocorreu um erro</p></div>';
            }
        }

        $this->data['result'] = $this->ti_model->getByIdParque($this->uri->segment(3));

        
        $this->data['anexos'] = $this->ti_model->getAnexos($this->uri->segment(3));
        $this->data['anotacoes'] = $this->ti_model->getAnotacoes($this->uri->segment(3));

       

        

        $this->data['view'] = 'ti/editarParque';
        return $this->layout();
    }
    
    
    public function excluirParque()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'dOs')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para excluir PC');
            redirect(base_url());
        }

        $id = $this->input->post('id');
        $this->ti_model->delete('anexos', 'os_id', $id);
        $this->ti_model->delete('parque', 'idParque', $id);
       

        log_info('Removeu uma PC. ID: ' . $id);
        $this->session->set_flashdata('success', 'PC excluída com sucesso!');
        redirect(site_url('ti/parque/'));
    }
    
    ///alimentando via VBScript
    
     public function adicionarVBS()
    {
        //Array ( 
        ...
        //[hardDrive] => SanDisk SSD PLUS 480GB ATA Device 
        //[mac] => 84:34:97:03:5C:67 )
           

           if(!empty($this->input->get('modelo'))){
              
                $data = [
                'dataInicial' => date('Y-m-d'),
                'status' => 'Em uso',
                'setor' => 'JAVA',
                'numeroSerie' => 'SISTEMA-JAVA',
                'marca' => (!empty($this->input->get('marca'))) ? $this->input->get('marca') : NULL,
                'modelo' => (!empty($this->input->get('modelo'))) ? $this->input->get('modelo') : NULL,
                'processador' => (!empty($this->input->get('processador'))) ? $this->input->get('processador') : NULL,
                'memory' => (!empty($this->input->get('memory'))) ? $this->input->get('memory') : NULL,
                'hardDrive' => (!empty($this->input->get('hardDrive'))) ? $this->input->get('hardDrive') : NULL,
                'mac' => (!empty($this->input->get('mac'))) ? $this->input->get('mac') : NULL,
                'office' => (!empty($this->input->get('office'))) ? $this->input->get('office') : NULL,
                'windows' => (!empty($this->input->get('windows'))) ? $this->input->get('windows') : NULL,
            ];
                
            if($this->ti_model->edit('parque', $data, 'mac', $this->input->get('mac')) == true) {
                    //redirect(base_url());
                    redirect('http://ti.dunice.net');
            } else {
                $this->ti_model->add('parque', $data, true);
                redirect(base_url());
                //redirect('http://ti.dunice.net');
            }
...
//        $this->data['view'] = 'ti/testeVBS';
//        return $this->layout();

    }
    
    //nova divisão de estoque de computadores
    public function adicionarPeriferico()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'aParque')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para adicionar');
            redirect(base_url());
        ...
        if ($this->form_validation->run('periferico') == false) {
            $this->data['custom_error'] = (validation_errors() ? true : false);
        } else {
            $dataInicial = $this->input->post('dataInicial');
            $dataFinal = $this->input->post('dataFinal');
            ...
            //valor referente ao laço de repetição para inclusao
            $i = 1;
            $data = [
                'dataInicial' => $dataInicial,
                'clientes_id' => $this->input->post('clientes_id'), //set_value('idCliente'),
                ...
                'setor' => (!empty($this->input->post('setor'))) ? $this->input->post('setor') : NULL,
                'tipoProduto' => (!empty($this->input->post('tipoProduto'))) ? $this->input->post('tipoProduto') : NULL,
                //'quantidade' => (!empty($this->input->post('quantidade'))) ? $this->input->post('quantidade') : NULL,
            ];
            
            
            if (is_numeric($id = $this->ti_model->add('parque', $data, true))) {
            $num = $this->input->post('quantidade');
                
            for ($i = 0; $i < $num-1; $i++){
                $this->ti_model->add('parque', $data, true);
            }
                $this->session->set_flashdata('success', 'Periferico adicionado com sucesso!');
                log_info('Adicionou uma periferico');
                redirect(site_url('ti/parque'));
            } else {
                $this->data['custom_error'] = '<div class="alert">Ocorreu um erro.</div>';
            }
        ...
        $this->data['view'] = 'ti/adicionarPeriferico';
        return $this->layout();
    }
