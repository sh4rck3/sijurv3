...
class Tvinsert_model extends CI_Model
{
    ...
    public function getTv($table, $fields, $where = '', $perpage = 0, $start = 0, $one = false, $array = 'array')
    {
    ...
    public function getPerf($table, $fields, $where = '', $perpage = 0, $start = 0, $one = false, $array = 'array')
    {
       $this->db->select($fields);
       $this->db->from($table);
       $this->db->order_by('idPerf', 'desc');
       $this->db->limit($perpage, $start);
    ...
    ///get da tabela performance para edição
    
     public function getByIdPerf($id)
    {
        $this->db->select('AcomPerformance.*');
        $this->db->from('AcomPerformance');
        $this->db->where('AcomPerformance.idPerf', $id);
        $this->db->limit(1);
        return $this->db->get()->row();
    }
