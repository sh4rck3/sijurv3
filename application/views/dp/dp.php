...
<div class="span12" style="margin-left: 0">
    <form method="get" action="<?php echo base_url(); ?>index.php/dp/cdlPesquisa">
        <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'aDp')) { ?>

            
<div class="span12" style="margin-left: 0">
<div class="span12">
    <a href="<?php echo base_url(); ?>index.php/dp/adicionar" class="btn btn-success span6"><i class="fas fa-plus"></i> Adicionar Remessa CDL</a> 
    <a href="<?php echo base_url(); ?>index.php/dp/import" class="btn btn-success span6"><i class="fas fa-plus"></i> Importar Remessa CDL</a> 
            </div>
</div><br><br>
        <?php
        } ?>
        ...
        <div class="span3">
            <input type="text" name="pesquisa" id="pesquisa" placeholder="Nome do cliente ou Codigo Exyon  a pesquisar" class="span12" value="">
        </div>
...
<!--                        <th>Escritorio</th>-->
                ...
                <tbody>
                    ...
                    foreach ($results as $r) {
                        ...
                        //echo '<td>' . $r->NOME_DO_ESCRITORIO . '</td>';
                        ...
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vDp')) {
                            echo '<a style="margin-right: 1%" href="#modal-ver' . $r->idCdl . '" role="button" data-toggle="modal"  class="btn-nwe" title="Ver mais detalhes"><i class="bx bx-show"></i></a>';
                        }
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'eDp'))  {
                            echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/dp/editar/' . $r->idCdl . '" class="btn-nwe3" title="Editar"><i class="bx bx-edit"></i></a>';
                        }
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'dDp') ) {
                            echo '<a href="#modal-excluir" role="button" data-toggle="modal" os="' . $r->idCdl . '" class="btn-nwe4" title="Excluir"><i class="bx bx-trash-alt"></i></a>';
                        }
                        echo '</td>';
                        echo '</tr>';
                       } ?>
