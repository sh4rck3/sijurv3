...
<div class="span12" style="margin-left: 0">
    <form method="get" action="<?php echo base_url(); ?>index.php/bradesco/gerenciar">
        <?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'aBradesco')) { ?>
            <div class="span3">
        ...
        <div class="span2">
            <select name="status[]" id="status" class="span12" multiple>
                ...
                <option value="Odair Junior">Odair Junior</option>
                <option value="Erika Alencar">Erika Alencar</option>
...
<div class="widget-box">
    <div class="widget-title">
        ...
        <h5>Planilha de controle do Bradesco</h5>
    </div>
...
//                        }
                     ...
                     echo '<td><span class="badge" style="background-color: ' . $cor2 . '; border-color: ' . $cor2 . '">' . $r->OBS . '</span> </td>';
                        echo '<td>' . $r->situacao_iniciais . '</td>';
                       echo '<td>';
//
//                        $editavel = $this->os_model->isEditable($r->idOs);
//
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vBradesco')) {
                            echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/bradesco/visualizar/' . $r->idBradesco . '" class="btn-nwe" title="Ver mais detalhes"><i class="bx bx-show"></i></a>';
                        }
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'eBradesco')) {
                            echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/bradesco/editar/' . $r->idBradesco . '" class="btn-nwe3" title="Editar"><i class="bx bx-edit"></i></a>';
                        }
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'dBradesco')) {
                            echo '<a href="#modal-excluir" role="button" data-toggle="modal" os="' . $r->idBradesco . '" class="btn-nwe4" title="Excluir"><i class="bx bx-trash-alt"></i></a>  ';
                        }
                        echo '</td>';
                        //echo '</tr>';
                   } ?>
